...
<br>
  ...
  - ### Graph
    ...
    - <strong>BFS 에는 queue 를, DFS 에는 Stack 을 사용한다고 했을때 어떤면에서 이 자료구조들이 적합한지?</strong>
      - BFS는 어떤 노드에 연결되어있는 모든 노드들을 탐색해야하기 때문에 순차적으로 노드들을 넣을 수 있는 queue 를 사용하는 것
      - DFS는 어떤 노드에  연결되어있는 노드들에 이어진 노드들을 탐색해야하고 탐색에 실패했을경우 되돌아가야하기 때문에 Stack 을 사용
    
    <br>
    
    - <strong>DFS 에서 탐색했을때의 시간복잡도와 이유?</strong>    
      - 연결되어 있는 노드들에 이어진 노드들만 탐색하기 때문에 노드의 갯수 + 간선의 갯수
